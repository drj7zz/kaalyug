  ...
  return (
    <section className="w-[min(92%,1100px)] mx-auto mt-[80px]">
      <div className="text-center">
        <p className="text-[#71dfcc] text-[11px] font-semibold tracking-[0.14em]">GETTING STARTED</p>
        <h2 className="mt-[10px] text-[clamp(26px,3.4vw,38px)] tracking-[-1px] font-bold">Three steps. That's it.</h2>
        <p className="max-w-[520px] mx-auto mt-[12px] text-[#a3a5b0] text-[13px] leading-[1.7]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-[15px] mt-[34px]">
        {STEPS.map((step, index) => (
          <article key={step.title} className="glass p-[26px] rounded-[22px] relative">
            <span className="absolute top-[22px] right-[24px] text-[#3c3e47] text-[26px] font-bold">
              0{index + 1}
            </span>
            <div className="w-[44px] h-[44px] grid place-items-center rounded-[13px] text-[#71e5d1] bg-[rgba(113,229,209,0.08)] border border-[rgba(113,229,209,0.2)] text-[17px]">
              <i className={`fa-solid ${step.icon}`}></i>
            </div>
            <h3 className="mt-[18px] text-[15px] font-bold">{step.title}</h3>
            <p className="mt-[9px] text-[#9fa1ac] text-[12px] leading-[1.7]">{step.text}</p>
          </article>
        ))}
      </div>
    </section>
