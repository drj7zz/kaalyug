  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(await readApiError(res, "Unable to create account"));
      }
    } catch {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center pt-20">
      <div className="glass p-[30px] rounded-[24px] w-full max-w-[400px]">
        <h2 className="text-[23px] font-bold mb-[20px]">Create Account</h2>
        ...
        <p className="mt-[20px] text-xs text-[#858691] text-center">
          Already have an account? <a href="/login" className="text-[#58c7ff]">Login</a>
        </p>
      </div>
    </div>
  );
