...
export default function Admin() {
  ...
  useEffect(() => { const session = getSession(); fetch(`${apiUrl}/admin/dashboard`, { headers: { Authorization: `Bearer ${session?.token}` } }).then(async (response) => { if (!response.ok) throw new Error('Unable to load dashboard'); return response.json(); }).then(setData).catch((err) => setError(err.message)); }, []);
